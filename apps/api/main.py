...
def start_research(person: str = Form(...)):
    ...
    with get_session() as db:
        job = Job(id=job_id, person=person, status="queued",
...
def job_sources_fragment(job_id: str):
    with get_session() as db:
        sources = db.query(Source).filter(Source.job_id == job_id).order_by(Source.id.desc()).limit(50).all()
    rows = []
    for s in sources:
        rows.append(f"""
        <tr>
    ...
    table = "<table><thead><tr><th>Source</th><th>Kind</th><th>Title/URL</th><th>Date</th></tr></thead><tbody>" + "\n".join(rows) + "</tbody></table>"
...
def job_timeline_fragment(job_id: str):
    ...
    rows = []
    for e in events:
        # citations is stored as JSON string
        rows.append(f"""
        <tr>
          <td>{e.date or ""}</td>
          <td>{e.event_text}</td>
          <td>
            {"".join([f'<div><a href="'+c.get("url","")+'" target="_blank">'+(c.get("label","source"))+'</a></div>' for c in (e.citations or [])])}
          </td>
        </tr>""".strip())
    table = "<table><thead><tr><th>Date</th><th>Event</th><th>Citations</th></tr></thead><tbody>" + "\n".join(rows) + "</tbody></table>"
    return HTMLResponse(table)
